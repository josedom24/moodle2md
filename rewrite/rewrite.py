...
def getUrl(actividad):
    ...
    print "mod/url/view.php?id=%s -> %s"%(actividad.find("moduleid").text,docactivity.find("url/externalurl").text)

def getAssign(actividad):
    nomfich=getNombreFichero(actividad.find("title").text)
    print "mod/assign/view.php?id=%s -> %s"%(actividad.find("moduleid").text,"doc/"+nomfich)
...
def getResource(actividad):
    ...
    try:
        print "mod/resource/view.php?id=%s -> %s"%(actividad.find("moduleid").text,"files/"+fichero[0].find("filename").text)
    except:
        pass
...
def getPage(actividad):
    ...
    nomfich=getNombreFichero(actividad.find("title").text)
    print "mod/page/view.php?id=%s -> %s"%(actividad.find("moduleid").text,"doc/"+nomfich)
#    try:
